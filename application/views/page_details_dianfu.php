...
<!-- ui-dialog -->
<div id="dialog_simple" title="审核通过" style="font-size: 20px;color: #FF0000;">
    <p></p>
</div>

<div id="dialog_reject" title="申诉" style="display: none;">
    <div class="form-group" style="margin:0;">
        <select class="form-control" name="reject_reason" id="reject_reason">
            <option value="">==== 请选择申诉原因 ====</option>
            <?php foreach ($reject_reasons as $k => $val) { ?>
                <option value="<?php echo $k?>"><?php echo $val?></option>
            <?php } ?>
        </select>
    </div>
    <p>
        <div class="form-group" style="margin:0; display: none;" id="div-reject_reason_txt">
            <input placeholder="请填写申诉原因" class="form-control" type="text" name="reject_reason_txt" id="reject_reason_txt" value="" >
        </div>
    </p>
</div>
...
<script type="text/javascript">
    $(function () {
        ...
        $('#reject_reason').change(function () {
            if ($(this).val() == '99') {
                $('#div-reject_reason_txt').show();
            } else {
                $('#div-reject_reason_txt').hide();
            }
        });
        ...
        $('#dialog_reject').dialog({
            ...
            buttons: {
                ...
                "提 交": function () {
                    ...
                    var reason_txt = $("#reject_reason_txt").val();
                    if(!reason) {
                        //alert('请选择申诉原因');
                        $("#reject_reason").focus();
                        return;
                    } else if(reason == '99' && !reason_txt) {
                        //alert('请填写申诉原因');
                        $("#reject_reason_txt").focus();
                        return;
                    }
                    ...
                    ajax_request(
                        $('.btn-reject').data('url'),
                        {
                            task_type: $('.btn-reject').data('task-type'),
                            task_id: $('.btn-reject').data('id'),
                            buyer_id: $('.btn-reject').data('buyer-id'),
                            conclusion: $('.btn-reject').data('conclusion'),
                            reason: reason,
                            reject_reason: reason_txt ? reason_txt : '--'
                        },
                        function (e) {
                            if (e.code == CODE_SUCCESS) {
                                location.reload();
